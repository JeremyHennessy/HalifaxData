...
try{
 for(const [viewportName,viewport] of viewports){
  ...
  for(const [route,expectedTitle] of routes){
   ...
   if(!/^#f/i.test(state.theme_bg)&&!state.root_bg.includes('244')&&!state.root_bg.includes('255'))throw new Error(`${viewportName}/${route}: expected light root theme, got token=${state.theme_bg} root=${state.root_bg}`);
