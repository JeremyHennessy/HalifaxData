...
try{
  for(const [viewportName,viewport] of viewports){
    ...
    const provinceStats=await page.evaluate(()=>({
      ...
      communityFundingOpen:document.querySelector('[data-build022-disclosure="communityFunding"]')?.open,
      communityFundingNested:Boolean(document.querySelector('[data-build022-disclosure="communityFunding"] .b13-community-funding'))
    }));
    ...
    if(provinceStats.communityFundingOpen!==false||!provinceStats.communityFundingNested) throw new Error(`${viewportName}: community funding detail must remain available but collapsed by default`);
    ...
    report.views.push({viewport:viewportName,overviewStats,order,provinceStats,sourceRegistryStats,dimensions:{overview,benchmarks,sources}});
    await context.close();
  }
